...
flowchart TB
    %% External Clients
    Client[Client Apps\n(Mobile/Web)] -->|REST| APIGW[API Gateway]

    %% API Gateway Responsibilities
    APIGW -->|REST Proxy\nAuth, Rate Limit, Validation| TS[Transaction Service]
    ...
    subgraph TransactionService [Transaction Service]
        TSREST[/REST Endpoints\n(/transactions, /history, /statements)/]
        TSgRPC{{gRPC APIs}}
    ...
    LS[Ledger Service] -->|gRPC Query\n(transaction details)| TSgRPC
    TSgRPC --> LS
